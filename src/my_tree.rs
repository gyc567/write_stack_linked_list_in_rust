...
impl<T: Ord> cmp::Ord for BST<T> {
    fn cmp(&self, other: &Self) -> cmp::Ordering {
        match self {
            ...
            &BST::Leaf(ref lhs) | &BST::Branch(ref lhs, _, _) => match other {
...
impl<T: Ord + Clone> BST<T> {
    ...
    fn push_node(self, node: &BST<T>) -> BST<T> {
        match self {
            BST::Nil => node.clone(),
            BST::Branch(value, left, right) => match node {
                &BST::Nil => BST::Nil,
                &BST::Leaf(ref new_value) | &BST::Branch(ref new_value, _, _) => {
                    match new_value.cmp(&value) {
                        cmp::Ordering::Less => {
                            BST::Branch(value, Box::new(left.push_node(node)), right)
                        }
                        _ => BST::Branch(value, left, Box::new(right.push_node(node))),
                    }
                }
            },
            BST::Leaf(value) => match node {
                &BST::Nil => BST::Nil,
                &BST::Leaf(ref new_value) | &BST::Branch(ref new_value, _, _) => {
                    match new_value.cmp(&value) {
                        cmp::Ordering::Less => {
                            BST::Branch(value, Box::new(node.clone()), Box::new(BST::Nil))
                        }
    ...
    pub fn get(&self, v: &T) -> Option<T> {
        match self {
            ...
            &BST::Branch(ref value, ref left, ref right) => match v.cmp(value) {
    ...
    pub fn min(&self) -> Option<T> {
        match self {
            ...
            &BST::Branch(ref value, ref left, _) if left.deref() == &BST::Nil => {
                Some(value.clone())
            }
    ...
    pub fn max(&self) -> Option<T> {
        match self {
            &BST::Nil => None,
            &BST::Leaf(ref value) => Some(value.clone()),
            &BST::Branch(ref value, _, ref right) if right.deref() == &BST::Nil => {
                Some(value.clone())
            }
    ...
    pub fn delete(self, v: T) -> BST<T> {
        match self {
            BST::Nil => self,
            BST::Leaf(ref value) => match v.cmp(value) {
            ...
            BST::Branch(value, left, right) => {
                match v.cmp(&value) {
                    ...
                    cmp::Ordering::Less => {
                        BST::Branch(value, Box::new(left.delete(v)), right).downgrade()
                    }
                    cmp::Ordering::Greater => {
                        BST::Branch(value, left, Box::new(right.delete(v))).downgrade()
                    }
                }
            }
        }
...
mod test {
    ...
    fn test_complicated_value_bst() {
        ...
        assert_eq!(
            "Branch(10, Branch(5, Leaf(3), Nil), Branch(15, Branch(12, Nil, Leaf(14)), Nil))",
            format!("{:?}", t)
        )
    }
    ...
    #[test]
    fn test_delete_left() {
        let t: BST<i32> = BST::new().push(10).push(5).push(3).delete(3);

        let expected: BST<i32> = BST::new().push(10).push(5);

        assert_eq!(t, expected);

        let t: BST<i32> = BST::new().push(10).push(5).push(3).delete(5);

        let expected: BST<i32> = BST::new().push(10).push(3);
    ...
    fn test_delete_right() {
        let t: BST<i32> = BST::new().push(10).push(20).push(30).delete(30);

        let expected: BST<i32> = BST::new().push(10).push(20);

        assert_eq!(t, expected);

        let t: BST<i32> = BST::new().push(10).push(20).push(30).delete(20);

        let expected: BST<i32> = BST::new().push(10).push(30);
